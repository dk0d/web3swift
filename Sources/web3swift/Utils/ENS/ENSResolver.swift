...
//        lazy var defaultOptions: CodableTransaction = .emptyTransaction
//
...
//            guard let transaction = resolverContract.createReadOperation("supportsInterface", parameters: [interfaceID as AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
...
//            guard let transaction = resolverContract.createReadOperation("interfaceImplementer", parameters: [nameHash, interfaceID] as [AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            guard let result = try? await transaction.callContractMethod() else { throw Web3Error.processingError(desc: "Can't call transaction") }
...
//            guard let nameHash = NameHash.nameHash(node) else { throw Web3Error.processingError(desc: "Failed to get name hash") }
//            guard let transaction = resolverContract.createReadOperation("addr", parameters: [nameHash as AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            guard let result = try? await transaction.callContractMethod() else { throw Web3Error.processingError(desc: "Can't call transaction") }
//            guard let address = result["0"] as? EthereumAddress else { throw Web3Error.processingError(desc: "Can't get address") }
//            return address
//        }
...
//            options.to = resolverContractAddress
//            guard let nameHash = NameHash.nameHash(node) else { throw Web3Error.processingError(desc: "Failed to get name hash") }
//            guard let transaction = resolverContract.createWriteOperation("setAddr", parameters: [nameHash, address] as [AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
...
//            guard let nameHash = NameHash.nameHash(node) else { throw Web3Error.processingError(desc: "Failed to get name hash") }
//            guard let transaction = resolverContract.createReadOperation("name", parameters: [nameHash as AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            guard let result = try? await transaction.callContractMethod() else { throw Web3Error.processingError(desc: "Can't call transaction") }
...
//            options.to = resolverContractAddress
//            guard let nameHash = NameHash.nameHash(node) else { throw Web3Error.processingError(desc: "Failed to get name hash") }
//            guard let transaction = resolverContract.createWriteOperation("setName", parameters: [nameHash, name] as [AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            guard let result = try? await transaction.writeToChain(password: password) else { throw Web3Error.processingError(desc: "Can't send transaction") }
//            return result
//        }
//
...
//            guard let nameHash = NameHash.nameHash(node) else { throw Web3Error.processingError(desc: "Failed to get name hash") }
//            guard let transaction = resolverContract.createReadOperation("contenthash", parameters: [nameHash] as [AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            guard let result = try? await transaction.callContractMethod() else { throw Web3Error.processingError(desc: "Can't call transaction") }
...
//            options.to = resolverContractAddress
//            guard let nameHash = NameHash.nameHash(node) else { throw Web3Error.processingError(desc: "Failed to get name hash") }
//            guard let transaction = resolverContract.createWriteOperation("setContenthash", parameters: [nameHash, hash] as [AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
...
//            guard let nameHash = NameHash.nameHash(node) else { throw Web3Error.processingError(desc: "Failed to get name hash") }
//            guard let transaction = resolverContract.createReadOperation("ABI", parameters: [nameHash, contentType.rawValue] as [AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            guard let result = try? await transaction.callContractMethod() else { throw Web3Error.processingError(desc: "Can't call transaction") }
...
//            options.to = resolverContractAddress
//            guard let nameHash = NameHash.nameHash(node) else { throw Web3Error.processingError(desc: "Failed to get name hash") }
//            guard let transaction = resolverContract.createWriteOperation("setABI", parameters: [nameHash, contentType.rawValue, data] as [AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            guard let result = try? await transaction.writeToChain(password: password) else { throw Web3Error.processingError(desc: "Can't send transaction") }
//            return result
//        }
//
...
//            guard let nameHash = NameHash.nameHash(node) else { throw Web3Error.processingError(desc: "Failed to get name hash") }
//            guard let transaction = resolverContract.createReadOperation("pubkey", parameters: [nameHash as AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            guard let result = try? await transaction.callContractMethod() else { throw Web3Error.processingError(desc: "Can't call transaction") }
...
//            guard let nameHash = NameHash.nameHash(node) else { throw Web3Error.processingError(desc: "Failed to get name hash") }
//            guard let transaction = resolverContract.createWriteOperation("setPubkey", parameters: [nameHash, pubkeyWithoutPrefix.x, pubkeyWithoutPrefix.y] as [AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            guard let result = try? await transaction.writeToChain(password: password) else { throw Web3Error.processingError(desc: "Can't send transaction") }
//            return result
//        }
//
...
//            guard let nameHash = NameHash.nameHash(node) else { throw Web3Error.processingError(desc: "Failed to get name hash") }
//            guard let transaction = resolverContract.createReadOperation("text", parameters: [nameHash, key] as [AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            guard let result = try? await transaction.callContractMethod() else { throw Web3Error.processingError(desc: "Can't call transaction") }
...
//            options.to = resolverContractAddress
//            guard let nameHash = NameHash.nameHash(node) else { throw Web3Error.processingError(desc: "Failed to get name hash") }
//            guard let transaction = resolverContract.createWriteOperation("setText", parameters: [nameHash, key, value] as [AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            guard let result = try? await transaction.writeToChain(password: password) else { throw Web3Error.processingError(desc: "Can't send transaction") }
//            return result
//        }
//    }

...
//            guard let transaction = contract.createWriteOperation("claim", parameters: [owner as AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            return transaction
//        }
//
...
//            defaultOptions.from = from
//            defaultOptions.to = address
//            guard let transaction = contract.createWriteOperation("claimWithResolver", parameters: [owner, resolver] as [AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            return transaction
//        }
//
...
//            defaultOptions.from = from
//            defaultOptions.to = address
//            guard let transaction = contract.createWriteOperation("setName", parameters: [name] as [AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            return transaction
//        }
//
//        public func getReverseRecordName(address: EthereumAddress) async throws -> Data {
//            guard let transaction = contract.createReadOperation("node", parameters: [address] as [AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
...
//            guard let transaction = contract.createReadOperation("defaultResolver", parameters: [] as [AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            guard let result = try? await transaction.callContractMethod() else { throw Web3Error.processingError(desc: "Can't call transaction") }

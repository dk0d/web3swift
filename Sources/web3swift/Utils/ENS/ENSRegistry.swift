...
//            guard let transaction = registryContract.createReadOperation("owner", parameters: [nameHash as AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
...
//            guard let transaction = registryContract.createReadOperation("resolver", parameters: [nameHash as AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            guard let result = try? await transaction.callContractMethod() else { throw Web3Error.processingError(desc: "Can't call transaction") }
//            guard let resolverAddress = result["0"] as? EthereumAddress else { throw Web3Error.processingError(desc: "No address in result") }
//            return Resolver(web3: web3, resolverContractAddress: resolverAddress)
//        }
//
//        public func getTTL(node: String) async throws -> BigUInt {
//            guard let nameHash = NameHash.nameHash(node) else { throw Web3Error.processingError(desc: "Failed to get name hash") }
//            guard let transaction = registryContract.createReadOperation("ttl", parameters: [nameHash as AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            guard let result = try? await transaction.callContractMethod() else { throw Web3Error.processingError(desc: "Can't call transaction") }
...
//            if let contractAddress = registryContractAddress {
//                options.to = contractAddress
//            }
//            guard let nameHash = NameHash.nameHash(node) else { throw Web3Error.processingError(desc: "Failed to get name hash") }
//            guard let transaction = registryContract.createWriteOperation("setOwner", parameters: [nameHash, owner] as [AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
...
//            guard let transaction = registryContract.createWriteOperation("setSubnodeOwner", parameters: [nameHash, labelHash, owner] as [AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            guard let result = try? await transaction.writeToChain(password: password) else { throw Web3Error.processingError(desc: "Can't send transaction") }
//            return result
//        }
//
...
//            }
//            guard let nameHash = NameHash.nameHash(node) else { throw Web3Error.processingError(desc: "Failed to get name hash") }
//            guard let transaction = registryContract.createWriteOperation("setResolver", parameters: [nameHash, resolver] as [AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            guard let result = try? await transaction.writeToChain(password: password) else { throw Web3Error.processingError(desc: "Can't send transaction") }
//            return result
//        }
//
...
//            }
//            guard let nameHash = NameHash.nameHash(node) else { throw Web3Error.processingError(desc: "Failed to get name hash") }
//            guard let transaction = registryContract.createWriteOperation("setTTL", parameters: [nameHash, ttl] as [AnyObject], extraData: Data()) else { throw Web3Error.transactionSerializationError }
//            guard let result = try? await transaction.writeToChain(password: password) else { throw Web3Error.processingError(desc: "Can't send transaction") }
//            return result
//        }
//    }

    ...
    struct EIP67Code {
            ...
            public func toString() -> String? {
                let encoding = method + "(" + parameters.map { el -> String in
                    if let string = el.1 as? String {
                        ...
                        return el.0.abiRepresentation + " " + data.toHexString().add0x
                    }
                    return ""
                }.joined(separator: ", ") + ")"
                return encoding
        ...
        public func toString() -> String {
            ...
            if let data = data {
                switch data {
                case let .data(d):
                    queryItems.append(URLQueryItem(name: "data", value: d.toHexString().add0x))
                case let .function(f):

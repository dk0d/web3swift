...
public extension EthereumBloomFilter {
    ...
    internal static func bloom9(_ number: BigUInt) -> BigUInt {
    ...
    /// - Returns: Bloom filter.
    internal static func bloom9(_ data: Data) -> BigUInt {
    ...
    static func bloomLookup(_ bloom: EthereumBloomFilter, topic: Data) -> Bool {
        bloom.test(topic: topic)
    }

    static func bloomLookup(_ bloom: EthereumBloomFilter, topic: BigUInt) -> Bool {
    ...
    /// - Returns: `true` if topic is possibly in set, `false` if definitely not in set.
    func test(topic: BigUInt) -> Bool {
        test(topic: topic.serialize())
    }

    func lookup(_ topic: Data) -> Bool {
    ...
    /// - Returns: bloom filter.
    static func createBloom(_ receipts: [TransactionReceipt]) -> EthereumBloomFilter {
        var bin = BigUInt(0)
    ...
    mutating func add(_ biguint: BigUInt) {
    ...
    internal mutating func setBytes(_ data: Data) {

...
public extension TransactionMetadata {
    ...
    init(from decoder: Decoder) throws {
        ...
        blockHash = try container.decodeHexIfPresent(Data.self, forKey: .blockHash)
        transactionHash = try container.decodeHexIfPresent(Data.self, forKey: .transactionHash)
        transactionIndex = try container.decodeHexIfPresent(UInt.self, forKey: .transactionIndex)
        blockNumber = try container.decodeHexIfPresent(BigUInt.self, forKey: .blockNumber)
        gasPrice = try container.decodeHexIfPresent(BigUInt.self, forKey: .gasPrice)
    }
}

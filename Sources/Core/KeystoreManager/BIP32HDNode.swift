...
public class HDNode {
    ...
    public var parentFingerprint: Data = .init(repeating: 0, count: 4)
    public var childNumber: UInt32 = .init(0)
...
public extension HDNode {
    func derive(index: UInt32, derivePrivateKey: Bool, hardened: Bool = false) -> HDNode? {
        if derivePrivateKey {
            if hasPrivate { // derive private key when is itself extended private key
                var entropy: [UInt8]
                ...
                if index >= (UInt32(1) << 31) || hardened {
                    ...
                    let hmac: Authenticator = HMAC(key: chaincode.bytes, variant: .sha2(.sha512))
                ...
                if bn > HDNode.curveOrder {
                    if trueIndex < UInt32.max {
                        return derive(index: index + 1, derivePrivateKey: derivePrivateKey, hardened: hardened)
                    }
                    return nil
                }
                ...
                guard let fprint = try? RIPEMD160.hash(message: publicKey.sha256())[0 ..< 4] else {
                    return nil
                }
            ...
            guard pubKeyCandidate.bytes[0] == 0x02 || pubKeyCandidate.bytes[0] == 0x03 else { return nil }
            guard let newPublicKey = SECP256K1.combineSerializedPublicKeys(keys: [publicKey, pubKeyCandidate], outputCompressed: true) else { return nil }
    ...
    func derive(path: String, derivePrivateKey: Bool = true) -> HDNode? {
    ...
    func serializeToString(serializePublic: Bool = true, version: HDversion = HDversion()) -> String? {
        guard let data = serialize(serializePublic: serializePublic, version: version) else { return nil }
        let encoded = Base58.base58FromBytes(data.bytes)
        return encoded
    }

    func serialize(serializePublic: Bool = true, version: HDversion = HDversion()) -> Data? {
        var data = Data()

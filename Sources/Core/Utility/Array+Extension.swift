...
extension Array where Element: BinaryInteger {
    ...
    func mean() -> BigUInt? {
        ...
        return BigUInt(reduce(0, +)) / BigUInt(count)
    }
    ...
    func percentile(of value: Double) -> Element? {
        ...
        let normalizedValue = value / 100 * Double(count)
        let index = Int(ceil(normalizedValue))

        let sorted_data = sorted()
        guard index < count else { return sorted_data[sorted_data.count - 1] }

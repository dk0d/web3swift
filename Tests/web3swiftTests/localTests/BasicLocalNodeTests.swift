...
class BasicLocalNodeTests: LocalTestCase {
    ...
    func testDeployWithRemoteSigning() async throws {
        ...
        let receipt = try await web3.eth.transactionReceipt(txHash.drop0x.data(using: .utf8)!)
        print(receipt)
        ...
        let details = try await web3.eth.transactionDetails(txHash.drop0x.data(using: .utf8)!)
        print(details)
    }
